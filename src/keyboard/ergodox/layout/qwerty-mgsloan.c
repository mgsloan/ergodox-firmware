...
void kbfun_shift_delete(void) {
  bool lshift_pressed = _kbfun_is_pressed(KEY_LeftShift);
  bool rshift_pressed = _kbfun_is_pressed(KEY_RightShift);

  if (lshift_pressed | rshift_pressed) {
    _kbfun_press_release(true, KEY_DeleteForward);
    usb_keyboard_send();
    _kbfun_press_release(false, KEY_DeleteForward);
    usb_keyboard_send();
  } else {
    _kbfun_press_release(true, KEY_DeleteBackward);
    usb_keyboard_send();
    _kbfun_press_release(false, KEY_DeleteBackward);
    usb_keyboard_se();
  }
}

#define  shftdel  &kbfun_shift_delete
